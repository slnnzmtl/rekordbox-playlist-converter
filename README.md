...
Double-click **Rekordbox WAV Converter.app**. Defaults write to `~/Documents/rekordbox-wav` (a different folder than the CLI’s `./output`). First launch: right-click → **Open** if Gatekeeper blocks it. macOS may ask for Documents access when writing there.
...
1. Put **static** `ffmpeg` and `ffprobe` for your Mac’s arch (arm64 or x86_64) in `vendor/ffmpeg/`. Do **not** copy Homebrew’s binaries (they link cellar dylibs). Example source: [ffmpeg.martin-riedl.de](https://ffmpeg.martin-riedl.de/) release zips. Then:
...
2. Build with a Python that has Tk (`python3 -m tkinter` must open a window). Homebrew’s `python@3.x` often lacks `_tkinter` — use a framework build, or `brew install python-tk@3.12` and that interpreter:
...
The app lands in `dist/Rekordbox WAV Converter.app`. Ad-hoc sign if needed: `codesign --force --deep -s - "dist/Rekordbox WAV Converter.app"`.
