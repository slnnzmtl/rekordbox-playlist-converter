...
class RunFfmpegRewriteGateTests(unittest.TestCase):
    ...
    def test_run_ffmpeg_rewrites_when_output_is_extensible(self) -> None:
        """24-bit ffmpeg WAV is WAVE_FORMAT_EXTENSIBLE; rewrite to CDJ-safe."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            src = root / "src.wav"
            dest = root / "out.wav"
            write_pcm_wav(src, bits=24, sample_rate=48000)

            def write_extensible(_cmd: object, **_k: object) -> FakeProc:
                write_pcm_wav(
                    dest,
                    bits=24,
                    sample_rate=48000,
